...
async function scanStock(raw) {
  ...
  if (last.close > refHigh) msg += `🚀 突破近20日高點 ${refHigh}\n`;
  if (last.close < refLow) msg += `🧨 跌破近20日低點 ${refLow}\n`;
  if (ma20) {
...
async function chatWithOpenAI(message) {
  if (!OPENAI_API_KEY) return "未設定 OPENAI_API_KEY";
  ...
  return response.data.output_text || "沒有回應";
}
...
async function handleEvent(event) {
  ...
  // 一般聊天走 OpenAI
  const aiReply = await chatWithOpenAI(text);
